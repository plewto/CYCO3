;;;; CYCO plugins sj eastwest mor2 guitars.lisp
;;;;
;;;; Instruments:
;;;;
;;;; ew-guitar
...
(defun baritone (&key (parent ew-guitar) channel 
  ...
  (let ((inst (make-instrument 'baritone
    ...
    (make-eastwest-program-map inst '((sus         "Sustain down/up RR")
...
(defun baritone-rhythm (&key (parent ew-guitar) channel 
			     keynumber-map articulation-map dynamic-map
			     (remarks "Ministry of Rock Guitar"))
  (let ((inst (make-instrument 'baritone-rhythm
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((sus    "Pick sustain")
				      (sus2   "Pick sustain 2")
...
(defun carvin-bridge (&key (parent ew-guitar) channel 
			   keynumber-map articulation-map dynamic-map
			   (remarks "Ministry of Rock Guitar"))
  (let ((inst (make-instrument 'carvin-bridge
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((sus       "Sustain no-vibrato RR")
...
(defun carvin-bridge-rhythm (&key (parent ew-guitar) channel 
				  keynumber-map articulation-map dynamic-map
				  (remarks "Ministry of Rock Guitar"))
  (let ((inst (make-instrument 'carvin-bridge-rhythm
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((down   "Pick down")
				      (up     "Pick up")
...
(defun carvin-neck (&key (parent ew-guitar) channel 
			 keynumber-map articulation-map dynamic-map
			 (remarks "Ministry of Rock Guitar"))
...
(defun carvin-neck-rhythm (&key (parent ew-guitar) channel 
				keynumber-map articulation-map dynamic-map
				(remarks "Ministry of Rock Guitar"))
...
(defun jaguar (&key (parent ew-guitar) channel 
		    keynumber-map articulation-map dynamic-map
		    (remarks "Ministry of Rock Guitar"))
...
(defun jaguar-rhythm (&key (parent ew-guitar) channel 
			   keynumber-map articulation-map dynamic-map
			   (remarks "Ministry of Rock Guitar"))
  (let ((inst (make-instrument 'jaguar-rhythm
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((long   "Strum long RR, position 1")
...
(defun lespaul (&key (parent ew-guitar) channel 
		     keynumber-map articulation-map dynamic-map
		     (remarks "Ministry of Rock Guitar"))
  (let ((inst (make-instrument 'lespaul
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((long         "Long no vibrato RR")
...
(defun lespaul-rhythm (&key (parent ew-guitar) channel 
			    keynumber-map articulation-map dynamic-map
			    (remarks "Ministry of Rock Guitar"))
  (let ((inst (make-instrument 'lespaul-rhythm
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((long        "Long fast RR")
				      (slow        "Long slower RR")
				      (death       "Long death RR")
				      (hammer      "Hammer on/pull off  (link hammer slide)")
				      (slide       "Slide up/down       (link hammer slide)")
...
(defun schecter (&key (parent ew-guitar) channel 
		      keynumber-map articulation-map dynamic-map
		      (remarks "Ministry of Rock Guitar"))
  (let ((inst (make-instrument 'schecter
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((sus          "Sustain no-vibrato")
				      (vib          "Sustain with vibrato RR")
				      (motto        "Motto vibrato")
				      (hammer       "Hammer on/Pull off  (link hammer slide)")
				      (slide        "Slide up/down       (link hammer slide)")
...
(defun schecter-rhythm (&key (parent ew-guitar) channel 
			     keynumber-map articulation-map dynamic-map
			     (remarks "Ministry of Rock Guitar"))
  (let ((inst (make-instrument 'schecter-rhythm
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((long   "Long sustain RR")
				      (short  "Short RR")
				      (death  "Long death RR")
...
(defun thinline (&key (parent ew-guitar) channel 
		      keynumber-map articulation-map dynamic-map
		      (remarks "Ministry of Rock Guitar"))
...
(defun thinline-rhythm (&key (parent ew-guitar) channel 
			     keynumber-map articulation-map dynamic-map
			     (remarks "Ministry of Rock Guitar"))
  (let ((inst (make-instrument 'thinline-rhythm
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((pos1   "Position 1 RR")
				      (pos2   "Position 2 RR")
				      (noise  "Strum noise 1")
				      (noise2 "Strum noise 2")))
