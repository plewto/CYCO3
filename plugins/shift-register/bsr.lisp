;;;; CYCO shift-register bsr  (Binary Shift Register)
;;;;
;;;; 

(constant +bsr-length+ 16)

(defclass bsr (pattern)
  ((length
    :allocation :class
    :type integer
    :reader sr-length
   ...
   (prerun
    ...
    :initarg :prerun))
  (:documentation
   "Implements a 16-bit linear feedback binary shift register.
...
(defmethod shift-register-p ((sr bsr)) t)

(defmethod reset ((sr bsr))
  (setf (sr-register sr)(sr-seed sr))
  (dotimes (i (sr-prerun sr))(next-1 sr))
  sr)

(defun bsr (seed taps &key
...
(defmethod value ((sr bsr))
  (let ((mask (sr-mask sr))
	(base (sr-base sr))
	(bias (sr-bias sr)))
    (+ (rem (logand mask (sr-register sr)) base) bias)))
    
(defmethod shift-register-feedback ((sr bsr) &optional (insert 0))
...
(defmethod shift-1 ((sr bsr) &optional (insert 0))
  (let ((fb (shift-register-feedback sr insert)))
    (setf (sr-register sr)
	  (logior (ash (sr-register sr) 1) fb)))
  (value sr))

(defmethod next-1 ((sr bsr))
  (shift-1 sr 0))

(defmethod shift-register-period ((sr bsr) &key 
...
(defmethod dump ((sr bsr))
  (format t "BINARY-SHIFT-REGISTER~%")
