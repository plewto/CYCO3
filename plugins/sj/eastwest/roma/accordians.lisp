;;;; CYCO plugins sj eastwest roma accordians.lisp
;;;;
;;;; Instruments
;;;;
;;;;     bandoneon
...
(let ((accord-alist '((out-full   "out-left-full" )
  ...
  (defun campana (&key (parent ew-roma)(channel nil)
    ...
    (let ((inst (make-instrument 'campana
      ...
      (make-eastwest-program-map inst accord-alist)
      (param campana inst)
      inst))
  
  (defun excelsior (&key (parent ew-roma)(channel nil)
			 keynumber-map articulation-map dynamic-map
			 remarks)
...
(defun silvestri (&key (parent ew-roma)(channel nil)
		       keynumber-map articulation-map dynamic-map
		       remarks)
  (let ((inst (make-instrument 'silvestri
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '((out-full  "out-left-full")
				      (in-octave "in-left-octave")
				      (out-major "out-left-major")
				      (out-minor "out-left-minor")
				      (out-7     "out-left-7")))
...
(defun bandoneon (&key (parent ew-roma)(channel nil)
		       keynumber-map articulation-map dynamic-map
		       remarks)
  (let ((inst (make-instrument 'bandoneon
			       ...
			       :articulation-map articulation-map
			       :dynamic-map dynamic-map)))
    (make-eastwest-program-map inst '(sus sforzando portato short crescendo 
