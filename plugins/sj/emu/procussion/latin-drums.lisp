;;;; CYCO config sj emu/procussion/latin-drums
;;;;
...
(let* ((tim1-keys '((A  . (36 ""))
       ...
       (macho-keys '((tone    . (90 ""))
		      ...
		      (rim-3   . (89 ""))
		      (slap-4  . (96 ""))))
       (conga-keys '((a  . ( 98 "Slide Conga 1"))
		      (b  . (100 "Slide Conga 2"))
		      (c  . (101 "Slide Conga 2"))
		      (d  . (102 "Slide Conga 2"))))
       (shaker-keys '((a . (78 ""))
		       (b . (66 ""))))
       (composite-keys '((TIMBALE-A    . (36 ""))
			  (TIMBALE-B    . (40 ""))
			  (TIMBALE-C    . (42 ""))
			  (TIMBALE-D    . (37 ""))
			  (TIMBALE-E    . (39 ""))
  ...
  (defun latin-drums (&key (parent pro3)(channel nil) articulation-map dynamic-map)
    (let* ((latdrm (instrument latin-drums
      ...
      (instrument timbal-1
		  ...
		  :transient t
		  :keynumber-map (symbolic-keynumber-map tim1-keys))

      (instrument timbal-2 
		  :parent latdrm
		  :remarks "Procussion latin-drums Timbales 2"
		  :transient t
		  :keynumber-map (symbolic-keynumber-map tim2-keys))

      (instrument tumba 
		  :parent latdrm
		  :remarks "Procussion latin-drums Tumba drum"
		  :transient t
		  :keynumber-map (symbolic-keynumber-map tumba-keys))
      ...
      (instrument hembra 
		  ...
		  :transient t
		  :keynumber-map (symbolic-keynumber-map hembra-keys))

      (instrument macho 
		  :parent latdrm
		  :remarks "Procussion latin-drums macho drum"
		  :transient t
		  :keynumber-map (symbolic-keynumber-map macho-keys))

      (instrument conga 
		  :parent latdrm
		  :remarks "Procussion latin-drums Conga"
		  :transient t
		  :keynumber-map (symbolic-keynumber-map conga-keys))
