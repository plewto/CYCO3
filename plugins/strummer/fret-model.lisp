;;;; CYCO strummer plugin fret-model
;;;;
...
(defmethod is-chord ((fw fret-model)(obj t)) nil)

(defmethod is-chord ((fw fret-model)(chord-type symbol))
  (gethash chord-type (chord-table fw)))

(defmethod chord-family ((fw fret-model)(chord-type symbol))
  (or (is-chord fw chord-type)
      (cyco-warning (sformat "Chord type ~A is not defined for FRET-MODEL ~A"
			   chord-type (name fw)))))
...
(defmethod add-chord-type! ((fw fret-model)(chord-type symbol))
  (let ((cf (or (is-chord fw chord-type)
...
(flet ((illegal-positions-list (fw key positions)
				...
				(cyco-warning msg1 (sformat msg2 (name fw) key)
  ...
  (defmethod chord-variation! ((fw fret-model)(chord-type symbol)(key t)(positions vector))
	...
	(let* ((pos (aref positions i))
	       (kn (position->keynumber (aref sarray i) pos)))
...
(defmethod key-positions ((fw fret-model)(key t) &key (no-duplicates t))
...
Where s is string number 
  ...
  (let ((acc '())
	...
	(dolist (p (key-positions mc key))
	  (let ((kn (position->keynumber mc p)))
...
(flet ((define-solo-chords (fw key)
	 (let ((scount (fm-string-count fw))
	       (positions (key-positions fw key :no-duplicates t)))
	   (dolist (p positions)
	     (let ((str (second p))
		   (pos (sixth p))
...
(defmethod define-chord-family! ((fw fret-model) (chord-type symbol) &key
  ...
  (if (or (and cs df)(and ds ef)(and fs gf)(and gs af)(and as bf))
      (let ((msg1 "DEFINE-CHORD-FAMLY!  FRET-MODEL ~A")
	    (msg2  "Mutuly excluisive enharmonic key awrguments set."))
...
(defmethod format-neck ((fm fret-model)
  ...
  "Returns string representation of instrument neck.
   fm - A FRET-MODEL
   select - list of pitch classes todisplay, all other pitches are left blank."
  (let ((acc "   "))
