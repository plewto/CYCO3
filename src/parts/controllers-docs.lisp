...
;;;;
;;;; Docstrings for CONTROLLERS and BENDER classes.
;;;;
...
:no-thin     - Boolean, if true do not remove redundant controller events, default t.
...
(constant +bender-docstring+
	  "Creates new instance of BENDER part.
...
Commands fall into two general categories:
    ...
    1) Those that set the 'environment' include the following
       :reset :time :value :cycles :phase & :width

       The values set by these commands remain in effect until explicitly
       changed. 

   2) Those that generate events include
      :bend :ramp :saw :tri & :pulse
...
:value  - :VALUE start end
          ...
          The initial and final bend values.   Both are signed normalized
          floats.   -1.0 <= value <= +1.0

          The special value '*  reuses the current value.
          The special start value '<, sets the new start-value to the 
          current end-value.
...
:bend   - :bend time value

          Generates a single pitch-bend event for each instrument.
          time - must be in format accepted by the cue function.
          value - signed normalized float  -1.0 <= value <= +1.0
...
Generate single event at time (1 1 1) with value 0.5
...
Generate ramp between times (1 1 1) and (2 1 1) every 16th note
with initial value 0.0 and final value 1.0

        :time (1 1 1)(2 1 1) s :value 0 1.0 :ramp

Generate another ramp starting at the end-time of the previous one.

        :time < (3 1 1) s :ramp

        ;; Note the current values are reused.

Generate single-cycle triangle curve

       :time (1 1 1)(2 1 1) s :value -1.0 1.0 :cycle 1 :phase 0 :tri

Generate 2 pulse cycles with 33% pulse-width

      :time (1 1 1)(2 1 1) s :value 0 1.0 :cycle 2 :phase 0 :width .33 :pulse")
