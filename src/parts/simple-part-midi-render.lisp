...
(labels ((render-bend-events
	 ...
	 (playable-p 
	  (keynumber duration amplitude)
	  (not (and (rest-p keynumber)
		    (rest-p duration)
		    (rest-p amplitude))))
	 
	 (create-chord-template 
	  (chord-model chord-type keynumber inversion-degree octave)
	 ...
	 (expand-template
	  (chord-model template keynumber)
	 ...
	 (expand-chord 
	  (on-time key chord-type inversion octave articulation dynamic chord-model instrument)
	  (let ((acc '())
	 ...
	 (render-note-events
	  (on-time state chord-model instrument-list)
	  (let ((acc '())
  ...
  (defmethod render-once ((part simple-part) &key (offset 0))
	...
	(let* ((state-time (+ time-shift (simple-state-time state))))
