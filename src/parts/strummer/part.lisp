...
(defmethod clone ((source strummer) &key new-parent new-name)
  (let* ((name (->cyco-symbol (sformat (or new-name "~A")(name source))))
	 (parent (or new-parent (parent source)))
	 (new-part (make-strummer name (property source :instruments)
				  :section parent
    ...
    (let ((acc '()))
      (dolist (state (strummer-states source))
	(push (clone state) acc))
      (setf (strummer-states new-part)(reverse acc)))
