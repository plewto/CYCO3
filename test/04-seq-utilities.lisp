...
(let ((test-list '(A B C)))
  (pass? "->list 1" (equal (->list 'APE) '(APE)))
  (pass? "->list 2" (eq (->list test-list) test-list))
  (pass? "->list 3" (eq (final test-list) 'C))
  (pass? "->list 4" (equal (butfinal test-list) '(A B)))
  (pass? "->list 5" (and (eq (cnth 1 test-list)(cnth 4 test-list))))
  (pass? "slice 1" (equal (slice test-list 0) test-list))
  (pass? "slice 2" (equal (slice test-list 1) (cdr test-list)))
  (pass? "slice 3" (equal (slice test-list 0 -1) (butfinal test-list)))
  (pass? "flatten1" (equal (flatten1 '(A B (C D (E)))) '(A B C D (E))))
  (pass? "flatten" (equal (flatten '(A B (C (D)))) '(A B C D)))
  (pass? "rotate +1 " (equal (rotate test-list +1) '(B C A)))
  (pass? "rotate -1" (equal (rotate test-list -1) '(C A B)))
  (pass? "zip" (equal (zip '(A B C) '(E F G)) '(A E B F C G)))
  (pass? "palindrome" (equal (palindrome '(A B C)) '(A B C C B A)))
  (pass? "range" (= 55 (apply #'+ (range 0 11))))
  (pass? "fill-list" (equal (fill-list '(A nil C) '(W X Y Z)) '(A X C Z))))

(multiple-value-bind (head tail)(split-list '(:ape 1 2 :bat 3 4 :cat 5 6))
  (pass? "split-list" (and (equal head '(:ape 1 2))
...
(let ((test-list '(A B C))
      (test-vector #(X Y Z)))
  (pass? "->vector 1" (let ((rs (->vector 'A)))
			(and (vectorp rs)
			     (= (length rs) 1)
			     (eq (aref rs 0) 'A))))
  (pass? "->vector 2" (let ((rs (->vector test-list)))
			(and (vectorp rs)
			     (= (length rs)(length test-list))
			     (eq (nth 0 test-list)(aref rs 0))
			     (eq (nth 1 test-list)(aref rs 1))
			     (eq (nth 2 test-list)(aref rs 2)))))
  (pass? "->vector 3" (eq (->vector test-vector) test-vector)))
